...
const sendMail = async (req, res, next) => {
  ...
  const emailHtml = _helpers.emailTemplete.VercelInviteUserEmail({
  ...
  try {
    await _helpers.mailHelper.sendMail(options);
    return res.status(200).json({
      success: true,
      message: "The Email Has Been Sent Succefully"
    });
  } catch (err) {
    console.log("The error in sending mail is: ", err);
    return next((0, _httpErrors.default)(400, "Email Could not be sent!"));
  }
}; // /**
