...
async function getTransporter() {
  ...
  if (NODE_ENV !== 'development') {
    const testAccount = await (0, _nodemailer.createTestAccount)();
...
async function sendMail(mail) {
  ...
  const transporter = await getTransporter(); // Send mail

  try {
    const mailInfo = await transporter.sendMail(payload);
    return mailInfo;
  } catch (err) {
    console.log("There was an error sending and email", err);
